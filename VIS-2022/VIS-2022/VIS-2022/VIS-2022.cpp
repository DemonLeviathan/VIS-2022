...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		//In::printTable(in.words); // вывести в консоль изначальный текст

		LA::LEX tables;
		bool lex_ok = LA::analyze(tables, in, log, parm);					//выполнить лексический анализ
		...
		Generation::CodeGeneration(tables, parm, log);								//выполнить генерацию кода
	...
	{
		cout << e.message << endl << "line "<< e.inext.line << endl << "column " << e.inext.col;
	}
}
