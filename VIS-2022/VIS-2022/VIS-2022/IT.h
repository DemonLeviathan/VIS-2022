...
#define BOOL_DEFAULT 1				//значение по умолчанию для bool
...
#define NUM_MAXSIZE   4294967295		//максимальное значение для типа number
#define NUM_MINSIZE		0	//минимальное значение для типа number
...
#define LENGTH_PARAMS_CNT 1				//кол-во параметров у функции lenght
#define STRTON_PARAMS_CNT 1				//кол-во параметров у функции strtonumber
...
namespace IT
{	
	enum IDDATATYPE { NUM = 1, STR = 2, BOOL = 3,  PROC = 4, UNDEF };//типы данных идентификаторов: числовой, строковый, логический, без типа(для процедур), неопределенный
	enum IDTYPE { V = 1, F = 2, P = 3, L = 4, S = 5 };	//типы идентификаторов: переменная, функция, параметр, литерал, стандартная функция
	enum STDFNC { F_CONCAT, F_LENGTH, F_STRTON, F_NOT_STD };	//стандартные функции
	static const IDDATATYPE CONCAT_PARAMS[] = { IT::IDDATATYPE::STR, IT::IDDATATYPE::STR };//параметры функции  concatstr
	static const IDDATATYPE LENGTH_PARAMS[] = { IT::IDDATATYPE::STR };//параметры функции strlen
