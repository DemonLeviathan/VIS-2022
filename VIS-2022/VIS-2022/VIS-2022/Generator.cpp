...
namespace Generation
{
	...
	string genCallFuncCode(LA::LEX& tables, Log::LOG log, int i)
	{
		string str;
	...
	string genEqualCode(LA::LEX& tables, Log::LOG& log, int i)
	{
		string str;
	...
	string genFunctionCode(LA::LEX& tables, int i, string funcname, int pcount)
	{
		string str;
		...
		while (LEXEMA(j) != LEX_RIGHTTHESIS) // пока параметры не кончатся
		{
			...
			{
				if (IT::IDDATATYPE::NUM)
					str = str + string(ITENTRY(j).id) + (ITENTRY(j).iddatatype == IT::IDDATATYPE::NUM ? " : sdword, " : " : dword, ");
				else if (IT::IDDATATYPE::BOOL)
					str = str + string(ITENTRY(j).id) + (ITENTRY(j).iddatatype == IT::IDDATATYPE::BOOL ? " : sdbyte, " : " : dbyte, ");
			}
			j++;
		}
		int f = str.rfind(',');
	...
	string genExitCode(LA::LEX& tables, int i, string funcname, int pcount)
	{
	...
	string genConditionCode(LA::LEX& tables, int i, string& cyclecode)
	{
		string str;
		conditionnum++;
		cyclecode.clear();
	...
	vector <string> startFillVector(LA::LEX& tables)
	{
		vector <string> v;
		v.push_back(BEGIN);
		v.push_back(EXTERN);
		...
		for (int i = 0; i < tables.idtable.size; i++)// const, data
		{
			...
			if (tables.idtable.table[i].idtype == IT::IDTYPE::L)	// литерал - в .const
			{
				...
				case IT::IDDATATYPE::BOOL: str = str + " byte '" + itoS(e.value.vbool); break;
				}
				vlt.push_back(str);
			}
			else if (tables.idtable.table[i].idtype == IT::IDTYPE::V)// переменная - в .data
			{
				...
				case IT::IDDATATYPE::BOOL: str = str + " byte 0"; break;
				}
				vid.push_back(str);
			}
		}
	...
	void CodeGeneration(LA::LEX& tables, Parm::PARM& parm, Log::LOG& log)
	{
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			switch (LEXEMA(i))
			{
				...
				case LEX_BRACELET:	// переход на метку в конце условия
				{
					if (LEXEMA(i + 1) == LEX_ISFALSE || LEXEMA(i + 1) == LEX_ISTRUE)
						str = str + "jmp next" + itoS(conditionnum);
				}
				case LEX_DIEZ:		// поставить метки в конце условия
				{
